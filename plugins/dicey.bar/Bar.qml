...
Item {
  ...
  // Third-party bars are created by an asynchronous Loader and receive these
  // values immediately afterward from omarchy-shell. They cannot be `required`
  // here: QML validates required properties before Loader.onLoaded can inject
  // them, which prevents the bar from starting after a shell restart.
  ...
  // Third-party bars receive their host context just after the asynchronous
  // Loader constructs them. Do not create module slots until then: otherwise
  // they resolve against an empty registry once and reserve blank bar space.
  readonly property bool hostReady: omarchyPath !== "" && barWidgetRegistry !== null
    && Util.isPlainObject(barConfig) && Object.keys(barConfig).length > 0
  ...
  // Separate the floating islands from the top edge, matching the old Waybar
  // treatment without affecting bars placed on other edges.
  readonly property int topIslandMargin: Style.space(5)
  ...
  component BarPanel: PanelWindow {
    ...
    margins {
      top: root.position === "top" ? root.topIslandMargin + (root.barHidden ? -root.barSize : 0) : 0
    ...
    // Horizontal bars use floating islands, so their layer surface stays
    // clear. Keep the standard solid surface for vertical bars, whose module
    // layout is intentionally inherited unchanged from the stock plugin.
    color: root.vertical && !root.transparent ? root.background : "transparent"
  ...
  // A compact variant of CenterModules for the floating island. Unlike the
  // stock full-width center region, this item sizes itself to its widgets. It
  // still gives the configured centerAnchor equal room on either side, so the
  // anchor (normally the clock) stays at the physical center of the display.
  component IslandCenterModules: Item {
    id: islandCenterRoot
    ...
    implicitWidth: hasAnchor
      ...
      : allCenterModules.width
    implicitHeight: hasAnchor
      ? Math.max(beforeAnchor.height, centerAnchorModule.height, afterAnchor.height)
      : allCenterModules.height
    width: implicitWidth
    ...
    ModuleSlot {
      id: centerAnchorModule
      visible: islandCenterRoot.hasAnchor
      entry: islandCenterRoot.anchorEntry
      region: "center"
      anchors.horizontalCenter: parent.horizontalCenter
      anchors.verticalCenter: parent.verticalCenter
    }
